...
const props = defineProps<{ users: User[]; roleOptions: RoleOption[] }>();
const page = usePage();
        ...
        <div class="grid gap-4 xl:grid-cols-[minmax(0,1fr)_20rem]">
                    ...
                    <article v-for="user in users" :key="user.id" class="p-4">
                        <template v-if="editingId === user.id">
                            <form class="grid gap-4" @submit.prevent="updateUser(user)">
                                ...
                                <div><Label :for="`role-${user.id}`">Profilo di accesso</Label><Select :id="`role-${user.id}`" v-model="editForm.role" class="mt-2 w-full" :options="roleOptions" option-label="label" option-value="value" :disabled="user.id === currentUserId" /><p v-if="user.id === currentUserId" class="mt-1 text-xs text-muted-foreground">Il tuo profilo non può essere ridotto da questa schermata.</p><InputError :message="editForm.errors.role" /></div>
                        ...
                        <template v-else>
                            <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div class="flex min-w-0 items-center gap-3"><span class="grid h-10 w-10 shrink-0 place-items-center rounded-full bg-[#e7f0e6] font-serif text-lg text-[#476246]">{{ user.name.slice(0, 1).toUpperCase() }}</span><div class="min-w-0"><div class="flex flex-wrap items-center gap-2"><h3 class="font-bold">{{ user.name }}</h3><span class="rounded-full bg-[#e5edf1] px-2 py-0.5 text-[11px] font-bold text-[#476774]">{{ roleOptions.find((role) => role.value === user.role)?.label ?? user.role }}</span></div><p class="truncate text-sm text-muted-foreground">{{ user.email }}</p></div></div><div class="flex gap-2"><button type="button" class="rounded-lg border border-border px-3 py-2 text-sm font-semibold hover:bg-muted" @click="startEditing(user)">Modifica</button><button v-if="user.id !== currentUserId" type="button" class="rounded-lg px-3 py-2 text-sm font-semibold text-destructive hover:bg-destructive/10" @click="deleteUser(user)">Elimina</button></div></div>
                        </template>
                    </article>
                </div>
            </section>
            ...
            <aside class="h-fit rounded-xl border border-border bg-card p-4 shadow-sm">
                ...
                <form class="mt-4 grid gap-4" @submit.prevent="createUser">
                    ...
                    <div><Label for="new-role">Profilo di accesso</Label><Select id="new-role" v-model="createForm.role" class="mt-2 w-full" :options="roleOptions" option-label="label" option-value="value" /><InputError :message="createForm.errors.role" /></div>
