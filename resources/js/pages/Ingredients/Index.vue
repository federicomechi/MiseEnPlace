...
function applyFilters(): void {
    router.get(
        '/operativita/ingredients',
        { search: search.value || undefined, bar: barOnly.value || undefined },
...
function formatCost(cost: string | null, unit: string | null): string {
    return cost
        ? `${Number(cost).toLocaleString('it-IT', { style: 'currency', currency: 'EUR' })}${unit ? ` / ${unit}` : ''}`
        : '—';
}
...
<template>
    ...
    <div class="mx-auto flex w-full max-w-7xl flex-1 flex-col gap-4 p-3 md:p-5">
        ...
        >
            <div>
                ...
                <p class="mt-0.5 text-sm text-muted-foreground">
                    Catalogo, disponibilità bar e costi di acquisto.
                </p>
            </div>
            <div class="flex flex-wrap gap-2">
                ...
                <Button
                    as="a"
                    href="/operativita/ingredients/create"
                    label="Nuovo ingrediente"
                    icon="pi pi-plus"
                />
            </div>
        </header>
        ...
        >
            ...
            >
                ...
                <Link
                    ...
                    >Crea il primo ingrediente</Link
                >.
            </div>
            <template v-else>
                    ...
                    <table class="w-full text-left text-sm">
                        ...
                        >
                            <tr>
                                ...
                                <th class="px-3 py-3">Fornitore corrente</th>
                        ...
                        <tbody class="divide-y divide-border">
                            ...
                            >
                                <td class="px-4 py-3">
                                    <Link
                                        :href="`/operativita/ingredients/${ingredient.id}/edit`"
                                ...
                                <td class="px-3 py-3 text-muted-foreground">
                                    <span>{{
                                        ingredient.supplier ?? 'Nessun listino'
                                    }}</span
                                    ><span
                                        v-if="ingredient.available_for_bar"
                                        class="ml-2 rounded-full bg-[#e9e6f7] px-2 py-0.5 text-[10px] font-bold text-[#67548e]"
                                        >BAR</span
                                    >
                                </td>
                                <td
                                ...
                                <td class="px-4 py-3 text-right">
                                    <Link
                                        :href="`/operativita/ingredients/${ingredient.id}/edit`"
                                        class="text-xs font-bold text-[#476246]"
                                        >Modifica</Link
                                    ><button
                                        type="button"
                ...
                <div class="divide-y divide-border md:hidden">
                    ...
                    >
                        ...
                        <p class="mt-1 text-xs text-muted-foreground">
                            {{
                                ingredient.supplier ??
                                'Nessun fornitore/listino corrente'
                            }}
                        </p>
                    </article>
                </div>
        ...
        >
            <span
                >{{ ingredients.from }}–{{ ingredients.to }} di
                {{ ingredients.total }}</span
            >
